...
10000->生成RDB，1000->RDB，这个根据自己的应用和业务的数据量，你自己去决定
...
5) 确认数据恢复，直接在命令行热修改redis配置，`config set appendonly yes`打开aof，这个redis就会将内存中的数据对应的日志，写入aof文件中,此时aof和rdb两份数据文件的数据就同步了
